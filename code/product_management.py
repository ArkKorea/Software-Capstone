...
def product_management_screen(page: ft.Page):
    return ft.View(
        ...
        controls=[
            # AppBar 추가
            ft.AppBar(
                title=ft.Text("내   상 품   관 리", size=22, weight=ft.FontWeight.BOLD),
                center_title=True,
                bgcolor=ft.Colors.WHITE,
                leading=ft.IconButton(
                    icon=ft.Icons.ARROW_BACK,
                    on_click=lambda _: page.go("/home")  # 이전 페이지 경로에 맞게 수정
                )
            ),
            ...
            ft.Column(
                controls=[
                    # 이미지 1개 (간격 없음)
                    ft.Container(
                        padding=ft.Padding(top=10, left=20, right=20, bottom=0),
                        content=ft.Row(
                            controls=[
                                ft.Image(
                    ...
                    ft.Container(
                        ...
                        margin=ft.Margin(left=0, right=0, top=-80, bottom=0),
                        content=ft.Container(
                            padding=ft.Padding(top=20, left=20, right=20, bottom=60),
                            content=ft.Column(
                                controls=[
                                    # 상품 등록 텍스트
                                    ...
                                    ft.Row(
                                        controls=[
                                            ...
                                            product_card(
                                                ...
                                                width_ratio=0.45,
                                                height_multiplier=1.2,
                                                on_click=lambda e: page.go("/myproductgroup")
                                            )
                                        ],
                                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN
                                    )
