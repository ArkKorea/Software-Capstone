...
def add_diet_screen(page: ft.Page):
    ...
    date_text = ft.Text(
        selected_date.current.strftime("%b %d, %Y"),
        size=16,
        color=ft.colors.GREEN
    )
    time_text = ft.Text(
        selected_time.current.strftime("%I:%M %p"),
        size=16,
        color=ft.colors.GREEN
    )
    ...
    def on_date_change(e):
        if date_picker.value:
            ...
            date_text.value = selected_date.current.strftime("%b %d, %Y")
            page.update()
    ...
    def on_time_change(e):
        if time_picker.value:
            ...
            time_text.value = selected_time.current.strftime("%I:%M %p")
            page.update()
    ...
    def save_diet(e):
        print(f"[저장됨] 날짜: {selected_date.current}, 시간: {selected_time.current.strftime('%H:%M')}, 식단: {added_foods}")
        page.go(f"/dietmanagement?date={selected_date.current.isoformat()}")

    # --- [5] 뒤로가기 버튼 핸들러 ---
    def go_back_with_overlay(e):
        origin = qs.get("from", [None])[0]
