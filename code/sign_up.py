...
def is_valid_password(password, email):
    ...
    email_id = extract_email_id(email)
    if email_id and email_id in password:
...
def is_valid_birth(birth):
    match = re.match(r"^(\d{4})-(\d{2})-(\d{2})$", birth or "")
    if not match:
        return False
    year, month, day = map(int, match.groups())
    return 1 <= month <= 12 and 1 <= day <= 31

def sign_up_screen(page: ft.Page):
    ...
    terms_1_row = ft.Row([terms_1, ft.TextButton("보기", on_click=lambda e: page.go("/terms1"))], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
    terms_2_row = ft.Row([terms_2, ft.TextButton("보기", on_click=lambda e: page.go("/terms2"))], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
    terms_3_row = ft.Row([terms_3, ft.TextButton("보기", on_click=lambda e: page.go("/terms3"))], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
    ...
    return ft.View(
        ...
        controls=[
            dialog_modal,
            ft.AppBar(
                title=ft.Text("회 원 가 입", size=22, weight=ft.FontWeight.BOLD),
                center_title=True,
            ...
            ft.Column(
                [
                    ...
                    terms_all,
                    ft.Container(content=terms_1_row, padding=ft.Padding(top=0, bottom=0, left=20, right=0)),
                    ft.Container(content=terms_2_row, padding=ft.Padding(top=0, bottom=0, left=20, right=0)),
                    ft.Container(content=terms_3_row, padding=ft.Padding(top=0, bottom=0, left=20, right=0)),
